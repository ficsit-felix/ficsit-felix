...
class ModelHelper {
    ...
    isLoading = false;
    requestedModels: Map<string, ((value?: THREE.BufferGeometry | PromiseLike<THREE.BufferGeometry> | undefined) => void)[]> = new Map();
    ...
    loadModel(path: string): Promise<THREE.BufferGeometry> {
        return new Promise((resolve, reject) => {
            if (this.loadedModels.has(path)) {
                ...
                if (this.requestedModels.has(path)) { // put request into
    ...
    loadFrame(): Promise<void> {
        ...
        if (!next.done) {
            ...
            this.loader.load(model,
                (gltf) => {
                    let geometry = (gltf.scene.children[0] as THREE.Mesh).geometry as THREE.BufferGeometry;
